#!/usr/bin/env python3
"""Generate Fence Posts' mod menu icon: the mod's own fence post model drawn
flat, textured from vanilla oak planks the way the model is.
...
OUT = os.path.join(HERE, "src/main/resources/assets/fence-posts/icon.png")
...
def build_icon():
    """The mod's own fence_post model is a 4x16x4 column textured from uv
    [6,0,10,16] of the plank texture. This draws that model flat: the front
    face, a shaded side face, and a lit top face."""
    planks = vanilla("block/oak_planks.png")
    canvas = blank(16, 16)
    for y in range(2, 16):
        for x in range(4, 9):
            canvas[y][x] = planks[y][6 + (x - 4) % 4]
    for y in range(2, 16):
        for x in range(9, 12):
            canvas[y][x] = shade(planks[y][6 + (x - 9) % 4], 0.68)
    for y in range(0, 2):
        for x in range(4, 12):
            canvas[y][x] = shade(planks[6 + y][6 + (x - 4) % 4], 1.20)
    # soft bevel on the outer columns so the post separates from the background
    for y in range(16):
        canvas[y][4] = shade(canvas[y][4], 0.72)
        canvas[y][11] = shade(canvas[y][11], 0.62)
